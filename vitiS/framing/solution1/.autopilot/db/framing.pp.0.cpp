...
extern "C" __attribute__((sdx_kernel("framing", 0))) void framing(hls::stream<fixed_point> &input_signal,
                               hls::stream<fixed_point> &output_frames) {
#line 15 "C:/Windows/vitiS/framing/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=framing
# 13 "framing/framing.cpp"
...
#pragma HLS PIPELINE
 ...
 static fixed_point buffer[128];

    VITIS_LOOP_21_1: while (!input_signal.empty()) {
        VITIS_LOOP_22_2: for (int i = 0; i < 128; i++) {
            buffer[i] = input_signal.read();
        }
        VITIS_LOOP_25_3: for (int i = 0; i < 128; i++) {
            output_frames.write(buffer[i]);
        }
    }
}
